...
import { generateUniqueInvoiceNumber } from "@/lib/invoice-generator"
...
export default function POSPage() {
  ...
  const [transactions, setTransactions] = useState<Transaction[]>([{ id: "1", cart: [], discount: 0, discountType: "percentage", customerName: "" }])
  ...
  const removeTransaction = (id: string) => {
    ...
    if (updatedTransactions.length === 0) {
      const newTransaction: Transaction = { id: Date.now().toString(), cart: [], discount: 0, discountType: "percentage", customerName: "" }
  ...
  const addNewTransaction = () => {
    ...
    const updatedTransactions = transactions.map(t => 
      t.id === activeTransactionId ? { ...t, cart, discount, customerName, discountType } : t
    )
  ...
  return (
    <div className="flex flex-col gap-6 h-[calc(100vh-7rem)]">
        ...
        <div className="flex gap-2 flex-1 overflow-x-auto">
          {transactions.map((transaction, index) => (
            <div key={transaction.id} className="relative group">
              <Button
                variant={activeTransactionId === transaction.id ? "default" : "outline"}
                size="sm"
                onClick={() => switchTransaction(transaction.id)}
                className="whitespace-nowrap"
              >
                Txn #{index + 1}
                <Badge className="ml-2 bg-orange-500">{transaction.cart.length}</Badge>
              </Button>
              <Button
                variant="ghost"
                size="icon"
      ...
      <div className="flex gap-6 h-full px-6 pb-6 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <Card className="border-border flex-1 flex flex-col min-h-0">
            <CardHeader className="pb-3">
              <CardTitle className="text-sm">Products ({filteredMedicines.length})</CardTitle>
            </CardHeader>
                    ...
                    {filteredMedicines.map((medicine) => (
                      ...
                      >
                        <div className="text-left w-full">
                          <div className="font-semibold text-xs line-clamp-2">{medicine.name}</div>
                          <div className="text-xs text-muted-foreground line-clamp-1 mt-1">{medicine.generic}</div>
                          <div className="flex justify-between items-center mt-2 gap-1">
                            <Badge variant="secondary" className="text-xs">${medicine.price.toFixed(2)}</Badge>
                            <span className={cn("text-xs font-medium", medicine.stock < 50 ? "text-orange-600" : "text-green-600")}>
                              {medicine.stock}
                            </span>
                          </div>
                        </div>
        ...
        <div className="w-96 flex flex-col min-w-0 gap-4">
          {/* Shopping Cart - Dynamic Height */}
          <Card className={cn("border-border flex flex-col overflow-hidden", cart.length > 0 ? "flex-1 min-h-0" : "")}>
            <CardHeader className="pb-3 flex-shrink-0">
              <CardTitle className="text-sm">Shopping Cart ({cart.length} items)</CardTitle>
            </CardHeader>
            {cart.length > 0 && (
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {cart.map((item) => (
                        ...
                        >
                          ...
                          <Select value={item.unit} onValueChange={(value) => updateUnit(item.id, value as "Piece" | "Strip" | "Box")}>
